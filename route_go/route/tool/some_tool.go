...
func Get_document_setting(db *sql.DB, doc_name string, set_name string, doc_rev string) [][]string {
    ...
    result := [][]string{}
    for rows.Next() {
        var set_data, revision string
        if err := rows.Scan(&set_data, &revision); err != nil {
            log.Printf("Get_document_setting: error scanning row: %v", err)
            continue // 또는 return [][], 상황에 따라 결정
        }
        result = append(result, []string{set_data, revision})
    }
...
func Get_setting(db *sql.DB, set_name string, data_coverage string) [][]string {
    ...
    if err != nil {
        // 적절한 에러 처리: 로그 출력, panic, 혹은 빈 결과 반환
        return [][]string{}
    }
    defer rows.Close()
    ...
    for rows.Next() {
        ...
        if err != nil {
            continue // 혹은 log.Printf("rows.Scan 실패: %v", err)
        }
