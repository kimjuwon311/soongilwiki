...
func main() {
	...
	if len(os.Args) >= 2 {
		...
		log.Println("포트 인자가 없어 기본 포트 3001 사용")
	}

	if len(os.Args) >= 3 {
		mode = os.Args[2]
	} else {
		log.Println("모드 인자가 없어 기본 모드 'prod' 사용")
	}

	// gin 초기화
	var r *gin.Engine
	if mode == "dev" {
		log.Println("개발 모드로 실행 중")
		r = gin.Default()
	} else {
		log.Println("운영 모드로 실행 중")
	...
	r.POST("/", func(c *gin.Context) {
		...
		if main_set["url"] == "test" {
			data, ok := main_set["data"]
			if !ok || data == "" {
				log.Println("Missing or empty 'data' field in request")
				c.JSON(http.StatusBadRequest, gin.H{
					"response": "error",
					"message":  "'data' field is required for test",
				})
				return
			}
	...
	// DB_init 방어 처리
			func() {
				defer func() {
					if r := recover(); r != nil {
						log.Printf("Recovered in DB_init: %v\n%s", r, debug.Stack())
		...
		if mode == "dev" {
			log.Println("Request URL:", main_set["url"])
		}
